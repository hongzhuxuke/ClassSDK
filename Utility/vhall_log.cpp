...
void InitLog(std::wstring logPath) {
  ...
  //const wchar_t *szPath = L"VhallClass\\ClassSDK";
  ...
  char FilePath[MAX_PATH] = { 0 };
  if (logPath.empty()) {
      const wchar_t *szPath = L"VhallClass\\Class";
      if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_APPDATA, NULL, 0, fullPath))) {
          // TcharToChar(szPath, fullPath);
          PathAppend(fullPath, szPath);
          TcharToChar(fullPath, FilePath);
      ...
      std::wstring logFullPath = logPath + L"\\VhallClass\\ClassSDK";
